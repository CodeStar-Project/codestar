...
public class MediaController {
    ...
    public ResponseEntity<ApiResponseDto<MediaUploadDto>> upload(@RequestParam("file") MultipartFile file, @AuthenticationPrincipal AuthenticatedUser principal) {
        if (principal == null) throw ApiException.unauthorized("Unauthenticated");
        String id = mediaStorage.store(file, principal.getId());
